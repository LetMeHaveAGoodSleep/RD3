...
namespace RD3.Controller
{
    public class TempController
    {
        ...
        {
            LogHelper.Debug($"执行温度时间序列项：[{item.StartTime}-{item.EndTime}]，操作值：{item.Value}");
            // 示例：设置SP值（根据实际业务替换）
            CurrentDeviceParameter.TempParam.SP = (float)item.Value;
            // 其他操作：如控制设备、记录日志等
        }
        ...
        {
            var timeSeries = CurrentDeviceParameter.TempParam.TimeSeries;
            var items = timeSeries.TimeSeriesItemCol.OrderBy(t => t.StartTime).ThenBy(t => t.EndTime).ToList();

            this._tsStartTime = DateTime.Now;
            if (CurrentDeviceParameter.TempParam.TimeSeries.TimeType == TimeType.RelativeTime)
            {
            ...
            _tsThread = new Thread(() =>
            {
                ...
                LogHelper.Debug($"{CurrentDeviceParameter.Name}的温度时间序列开始执行（{timeSeries.TimeType}模式）");
                ...
                while (!_shouldStop)
                {
                    try
                    {
                        ...
                        if (matchedItem != null)
                        {
                            ...
                            CurrentDeviceParameter.TempParam.TimeSeries.RunningInfo = $"时间序列运行到{currentStage}/{items.Count}阶段，该阶段已运行：{$"{timeSpan.Days:00}天{timeSpan.Hours:00}时{timeSpan.Minutes:00}分{timeSpan.Seconds:00}秒"}";
                            //LogHelper.Debug(CurrentDeviceParameter.TempParam.TimeSeries.RunningInfo);
                        ...
                        {
                            CurrentDeviceParameter.TempParam.TimeSeries.RunningInfo = $"温度时间序列已全部执行完成";
                            LogHelper.Debug($"{CurrentDeviceParameter.Name}的温度时间序列已全部执行完成");
                            _shouldStop = true;
                        }
                        else
                        {
                            TimeSpan timeSpan = TimeSpan.FromSeconds(currentMatchTime * 60);
                            CurrentDeviceParameter.TempParam.TimeSeries.RunningInfo = $"时间序列已运行：{$"{timeSpan.Days:00}天{timeSpan.Hours:00}时{timeSpan.Minutes:00}分{timeSpan.Seconds:00}秒"}";
                        }
                    }
                    catch (Exception ex)
                    {
                        LogHelper.Error($"{CurrentDeviceParameter.Name}的温度时间序列执行出错：{ex.Message}", ex);
                    }
                ...
                LogHelper.Debug($"{CurrentDeviceParameter.Name}的温度时间序列线程已停止");
            });
        ...
        public void StopTimeSeriesWork()
        {
            ...
            CurrentDeviceParameter.TempParam.TimeSeries.RunningInfo = string.Empty;
        }
    }
}
