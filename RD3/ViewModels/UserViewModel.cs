    ...
    internal class UserViewModel : NavigationViewModel, IDialogAware
    {
        ...
        public DelegateCommand<User> EditCommand => new((User user) =>
        {
            ...
            DialogHostService.ShowOnce(nameof(EditUserView), pairs, callback =>
            {
                ...
                user = callback.Parameters.GetValue<User>(nameof(User));
                UserCol[UserCol.IndexOf(user)] = user;
        ...
        public UserViewModel(IContainerProvider containerProvider, IDialogService dialog, IDialogHostService dialogHostService) : base(containerProvider, dialogHostService)
        {
            PageIndex = 1;
