...
namespace RD3.Shared
{
    public class RealCommandWrapper : ICommandWrapper
    {
        ...
        #region 0x0a 读MCU版本号
        public string GetMCUVersion(string insID, MCUBoardType boardType)
        {
            string version = string.Empty;
            SendCommand sendCommand = new SendCommand(CommandId.RMCUVersion, CommandExtendId.Read);
            sendCommand.SetValue(ParamId.RMCUVersion_Read_Type, (byte)boardType);
            try
            {
                RecvCommand recvCommand = Send(insID, sendCommand);
                if (recvCommand.GetExtCode() == CommandExtendId.ReadResponse)
        ...
        #region 0x0e 读写传感器校准
        public SensorCorrectParam GetSensorCorrect(string insID, SensorType sensorType)
        {
            ...
            sendCommand.SetValue(ParamId.RWSensorCorrect_Read_SensorType, (byte)sensorType);
            try
            {
                RecvCommand recvCommand = Send(insID, sendCommand);
                if (recvCommand.GetExtCode() == CommandExtendId.ReadResponse)
        ...
        public void SetMCUDownloadInfo(string insID, MCUBoardType boardType)
        {
            SendCommand sendCommand = new SendCommand(CommandId.RWMCUDownload, CommandExtendId.Write);
            try
            {
                sendCommand.SetValue(ParamId.RWMCUDownload_Write_No, (byte)boardType);
                RecvCommand recvCommand = Send(insID, sendCommand);
                if (recvCommand.GetExtCode() != CommandExtendId.WriteResponse)
                {
                    throw new Exception("设置失败");
