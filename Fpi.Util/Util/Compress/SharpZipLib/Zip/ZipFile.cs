...
namespace ICSharpCode.SharpZipLib.Zip
{
    ...
    public class ZipFile : IEnumerable, IDisposable
    {
        ...
        private long TestLocalHeader(ZipEntry entry, HeaderTest tests)
        {
            lock (baseStream_)
            {
                ...
                if (testHeader)
                {
                    ...
                    {
                        throw new ZipException(
                            string.Format("Version required to extract this entry is invalid ({0})", extractVersion));
                    }
                    ...
                    {
                        throw new ZipException(
                            string.Format("Patched data requires higher version than ({0})", extractVersion));
                    }
        ...
        {
            get { return updateNameTransform_; }

            set { updateNameTransform_ = value; }
        }
...
*/
                               ...
                               long bytesToCopy, bool updateCrc)
        {
            ...
            {
                throw new ZipException(
                    string.Format("Failed to copy bytes expected {0} read {1}", targetBytes, totalBytesRead));
            }
        ...
        private void ReadEntries()
        {
                ...
                (offsetOfCentralDir == 0xffffffff))
            {
                ...
                long offset =
                    LocateBlockWithSignature(ZipConstants.Zip64CentralDirLocatorSignature, locatedEndOfCentralDir, 0,
                                             0x1000);
                if (offset < 0)
                {
